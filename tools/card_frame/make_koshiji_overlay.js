// 越路フレーム overlay 生成（栃尾 make_tochio_overlay.js と同一方式）。
// 窓（中央の空クリーム）を透過し、下部シルエット帯は上端フェード→下端不透過の半透明ブレンド。
// 実測: 窓 left=210 / right=949 / top≈76、下部シーン上端≈1180。窓クリーム rgb≈(249,218,165)。
const sharp = require("sharp");
const RAW = "C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_koshiji_raw.png";
const OUT_OVL = "C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_koshiji_overlay.png";
const GCREAM = 210;  // 窓クリーム 緑ch≈217-221 → 210で安全
const WIN_YMAX = 1185;      // flood-fill the empty window down to here (just above the scenery footer)
const FOOT_X0 = 208, FOOT_X1 = 951;   // silhouette interior x-range (inside side borders)
const GRAD_TOP = 1180, GRAD_FULL = 1420; // alpha ramps 0 (top, blends w/ art) -> MAXA (solid)
...
(async()=>{
  ...
  console.log("koshiji overlay written. fully-transparent px="+t0+" (window). footer gradient 0->"+MAXA+".");
