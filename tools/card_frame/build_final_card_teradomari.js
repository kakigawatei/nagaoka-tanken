// 寺泊エリア用カード生成器（枠なしアート＋寺泊フレーム＋自動フィット縦ロゴ型押し）
// 山古志版と同一構造。フレームだけ frame_teradomari_overlay.png に差し替え。
// 窓は x210-951（左端は山古志/摂田屋と同じ＝ロゴ帯幅一致）、下部シルエット(日本海+佐渡島+魚市場通り)は不透過。
// usage: node build_final_card_teradomari.js "<artFileUrl>" "TERADOMARI ICHIBA" "<outHtml>" [offsetY]
// offsetY: アートを上へずらすpx（下部シルエットに主役が隠れる時に使う。window下端≈1314px）。
const fs = require('fs');
const [,, ART, NAME, OUT, OFFY] = process.argv;
const offY = parseInt(OFFY || '0', 10) || 0;
const FRAME = 'file:///C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_teradomari_overlay.png';
