// 蔵王エリア用カード生成器（枠なしアート＋蔵王フレーム＋自動フィット縦ロゴ型押し）
// 寺泊/山古志/栃尾/越路版と同一構造。フレームだけ frame_zao_overlay.png に差し替え。
// 窓は x212-955（左端は他エリアと同じ＝ロゴ帯幅一致）、下部シルエット(大けやき+信濃川+水道タンク+花手水+鳥居)は上端フェード。
// usage: node build_final_card_zao.js "<artFileUrl>" "KINPU JINJA" "<outHtml>" [offsetY]
// offsetY: アートを上へずらすpx（下部シルエットに主役が隠れる時に使う。window下端≈1080px）。
const fs = require('fs');
const [,, ART, NAME, OUT, OFFY] = process.argv;
const offY = parseInt(OFFY || '0', 10) || 0;
const FRAME = 'file:///C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_zao_overlay.png';
