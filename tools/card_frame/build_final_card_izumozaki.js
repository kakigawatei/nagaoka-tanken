// 出雲崎エリア用カード生成器（枠なしアート＋出雲崎フレーム＋自動フィット縦ロゴ型押し）
// 蔵王/寺泊/山古志/栃尾/越路/三島版と同一構造。フレームだけ frame_izumozaki_overlay.png に差し替え。
// 窓は x215-954（左ロゴ帯幅は他エリアとほぼ一致）、下部シルエット(丘陵＋花畑＋竪穴住居)はくっきり。
// usage: node build_final_card_izumozaki.js "<artFileUrl>" "RYOKAN KINENKAN" "<outHtml>" [offsetY]
// offsetY: アートを上へずらすpx（下部シルエットに主役が隠れる時に使う。window下端≈1300px）。
const fs = require('fs');
const [,, ART, NAME, OUT, OFFY] = process.argv;
const offY = parseInt(OFFY || '0', 10) || 0;
const FRAME = 'file:///C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_izumozaki_overlay.png';
