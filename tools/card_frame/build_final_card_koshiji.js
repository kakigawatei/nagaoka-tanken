// 越路エリア用カード生成器（枠なしアート＋越路フレーム＋自動フィット縦ロゴ型押し）
// 寺泊/山古志/栃尾版と同一構造。フレームだけ frame_koshiji_overlay.png に差し替え。
// 窓は x208-949（左端は他エリアと同じ＝ロゴ帯幅一致）、下部シルエット(里山+酒蔵+茅葺屋敷+川)は上端フェード。
// usage: node build_final_card_koshiji.js "<artFileUrl>" "HOTOKUSAN INARI" "<outHtml>" [offsetY]
// offsetY: アートを上へずらすpx（下部シルエットに主役が隠れる時に使う。window下端≈1180px）。
const fs = require('fs');
const [,, ART, NAME, OUT, OFFY] = process.argv;
const offY = parseInt(OFFY || '0', 10) || 0;
const FRAME = 'file:///C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_koshiji_overlay.png';
