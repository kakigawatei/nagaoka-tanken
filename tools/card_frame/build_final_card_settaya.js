// 摂田屋エリア用カード生成器（枠なしアート＋摂田屋フレーム＋自動フィット縦ロゴ型押し）
// 通常フレームとの違い：フレームが摂田屋仕様(frame_settaya_overlay.png)。
// 下部シルエットが不透過の"蔵並み"イラスト（生成り壁＋濃緑の屋根/煙突/杉玉）なので、
// 発光ハロー(make_halo)は不要＝アートは蔵並みのスカイラインまで自然に降りてくる。
// usage: node build_final_card_settaya.js "<artFileUrl>" "KINA SAFRAN" "<outHtml>"
const fs = require('fs');
const [,, ART, NAME, OUT] = process.argv;
const FRAME = 'file:///C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_settaya_overlay.png';
