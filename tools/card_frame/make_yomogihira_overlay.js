// よもぎ平フレーム overlay 生成（越路 make_koshiji_overlay.js と同一方式）。
// 窓（中央の空クリーム）を透過し、下部シルエット帯（山あいの温泉街・杉木立・鳥居・渓流）は上端フェード→下端不透過の半透明ブレンド。
// 実測: 窓 left=211 / right=949 / top≈76、窓クリーム rgb≈(247,212,155)、下部シーン最高tip≈1188。
const sharp = require("sharp");
const RAW = "C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_yomogihira_raw.png";
const OUT_OVL = "C:/Users/masat/Desktop/nagaoka-walk/tools/card_frame/frame_yomogihira_overlay.png";
const GCREAM = 204;  // 窓クリーム 緑ch≈209-212 → 204で安全
const WIN_YMAX = 1200;      // flood-fill the empty window down to here (just above the scenery footer, highest tip≈1188)
const FOOT_X0 = 208, FOOT_X1 = 951;   // silhouette interior x-range (inside side borders)
const GRAD_TOP = 1200, GRAD_FULL = 1450; // alpha ramps 0 (top, blends w/ art) -> MAXA (solid)
const BORDER_Y = 1510;     // keep the bottom outer border (below this) fully opaque
...
(async()=>{
  ...
  console.log("yomogihira overlay written. fully-transparent px="+t0+" (window). footer gradient 0->"+MAXA+".");
