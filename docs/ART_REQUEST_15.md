...
## 図鑑カードの「愛」兜クレスト（対応済み・豪華版のみ残）
- `card_kanetsugu_museum.png`（通常）＝**masaさん提供の「愛」漢字クレスト版に差し替え済み**（2026-07-12）。
- `card_kanetsugu_museum_deluxe.png`（豪華版）＝**まだ旧デザイン（ハート型クレスト）**。通常と揃えるため、
  同じ「愛」漢字クレストの兜で、**夜＋長岡花火＋金色の光**の豪華版を1枚生成して差し替えたい。
  参照＝新しい `card_kanetsugu_museum.png`。ファイル名は同じ `card_kanetsugu_museum_deluxe.png`。
