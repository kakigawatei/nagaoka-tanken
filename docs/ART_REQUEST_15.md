...
## A. 衣装「直江兼続（愛の兜）」のキャラスプライト（6枚）※リテイク中（2026-07-12）
...
## 図鑑カードの「愛」兜クレスト（Codexで通常＋豪華版を作り直し）
暫定で通常カードはmasa提供スクショの切り抜きに差し替え済みだが、前立が平面気味。
スプライトのリテイクと揃えて、**通常・豪華版とも Codex で立体の「愛」前立に作り直す**。
- 出力先＝Drive「14_新スポットカード」に同名上書き（`card_kanetsugu_museum.png` / `_deluxe.png`）、1024×1536。
- 参照＝同フォルダの既存カード（構図・アンティーク水彩トーン）。前立だけ立体金属の「愛」に置換。
- 通常＝昼。豪華版＝同構図を夜＋長岡花火＋金の光で豪華に。
- 取り込み時：768×1152 にリサイズして `assets/` へ、sw更新。
