...
4. ⚠️**masaへのChatGPTプロンプトは「プロンプト本体だけ」を単独メッセージ(コードブロックのみ)で出す。前置き・案内・注釈を同じメッセージに一切混ぜない。** masaがブロックをコピペしたとき余計な文がくっつかないように。「これを貼って👇」等の案内は必ず"別メッセージ"に分ける。1プロンプト＝1コードブロック＝1メッセージ。**トークンリセット後に忘れて再発しがち＝masaが何度も強く指摘。絶対に忘れない。** 詳細→memory [masa-chatgpt-prompt-format]。
...
7. **質問・確認は必ずDiscord（replyツールのテキスト）でスマホに届くように出す。例外なく常に。**（masa 2026-07-23の明確な指示）。地の文で書いて終わる／AskUserQuestionの選択肢UIを使う＝masaのスマホには届かない＝実質無視になる。何か選ばせたい・確認したいときは100%replyツールの本文テキストで文章として質問する。詳細は [masa-discord-ask-question メモリ] 参照。
...
1. **masaが実物の参照写真を1枚"先に"送る → Claudeがその写真を実際に見てから、その写真専用の"軽め"の昼プロンプト（§6-B）を作る。** 主役の細かい描写はしない＝構図・被写体・建物や木の形は"写真に完全におまかせ"、画風変換（水彩化）だけを強く効かせる。masaは**写真＋そのプロンプトを両方**ChatGPTに貼る。ロゴ名はスポット表から。（★2026-07-25 masa指摘で改訂：主役を細かく描写しすぎると、どんな写真を貼っても説明側が勝って"別物"が生成される。**写真先出し→写真を見て軽めプロンプト**が正解。）
...
### 6-B. 昼アート（★写真先出し→"軽め"版。2026-07-25改訂・以後こちらを標準に）
**流れ：masaが写真を先に送る → Claudeがその写真を見る → 写真固有の要点を1〜2個だけ足した"軽め"プロンプトを作る（主役は描写しすぎない＝写真におまかせ）。** 画風変換だけ強く、構図・被写体・形は「写真のとおり」を厳命する。`【 】`は写真を見て最小限だけ埋める。
```
添付した写真の場所を、この写真の構図・被写体・建物や木の形・位置関係を"そのまま活かして"、水彩の風景画に仕上げてください。写真を別の絵に描き替えるのではなく、写真をベースに画風だけを変えるイメージです。

【画風】温かみのある和の風景画。写実7：イラスト3くらいで少し絵画的に整える。やわらかな陰影、上質な和紙のような細かい粒状感、彩度は落ち着いたセピア寄り。自然光の昼、澄んだ空。日本画とレトロな観光ポスターの中間のような、品のある雰囲気。

【この写真で活かしてほしい点】【写真を見て1〜2個だけ：例「中央の◯◯を主役に」「手前の△△の質感」】

【変えないこと】写真の構図・被写体の形・位置関係・アスペクト比(縦長2:3)は変えない。要素を勝手に足したり減らしたりしない。

【禁止】画像内に文字・ロゴ・読める看板は入れない（後で加えます）。額縁や縁飾りも付けない。実在の人物は描かない（必要なら小さな後ろ姿のシルエット程度）。
```
※**参照写真が無い**スポットだけ、旧来の「主役を文章描写する自己完結プロンプト」を使う（下の旧テンプレを流用）。写真がある場合は必ず上の"軽め"版で。

<details><summary>（旧）主役を描写する自己完結版＝写真が無いとき用</summary>

```
新潟県長岡市の「【スポット名】」を1枚描いてください。【主役の特徴を文章で描写】。
【画風】温かみのある和の風景画。写実7：イラスト3。やわらかな陰影、和紙の粒状感、セピア寄り、昼の自然光。日本画とレトロ観光ポスターの中間。
【構図】縦長2:3。主役を中央〜やや上に大きく、手前に前景を置いて奥行き。
【禁止】文字・ロゴ・看板・額縁を入れない。実在人物は描かない。
```
</details>
